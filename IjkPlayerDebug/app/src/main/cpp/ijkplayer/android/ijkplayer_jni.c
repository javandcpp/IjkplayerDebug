...
static void message_loop_n(JNIEnv *env, IjkMediaPlayer *mp) {
    ...
    while (1) {
        ...
        LOGE("msg.what   find:%d   retval:%d",msg.what,retval);
        if (retval < 0)
            break;
        ...
        switch (msg.what) {
            ...
            case FFP_MSG_GET_IP:
                if (msg.obj) {
                    jstring info = (*env)->NewStringUTF(env, (char *) msg.obj);
                    post_event2(env, weak_thiz, MEDIA_IP, msg.arg1, msg.arg2, info);
                    J4A_DeleteLocalRef__p(env, &info);
                    LOGE("post evevt 30001");
                }
                break;
            default:
