...
#include "../../../../../../../../../Applications/android-ndk-r13/platforms/android-14/arch-arm/usr/include/math.h"
...
static void ksDebugMethod(char *result, void *ffp) {
#if defined(__ANDROID__)
    LOGE("ks probe:%s", result);
    int length = (int) strlen(result) + 1;
    FFPlayer *ffPlayer = (FFPlayer *) ffp;
    ffp_notify_msg4(ffPlayer, FFP_MSG_GET_IP, 1, 1, result, length);
#endif
}
