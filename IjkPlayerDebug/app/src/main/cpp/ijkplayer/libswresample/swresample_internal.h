...
struct DitherContext {
    ...
    struct AudioData noise;                                ///< noise used for dithering
    struct AudioData temp;                                 ///< temporary storage when writing into the input buffer isn't possible
...
typedef int     (* multiple_resample_func)(struct ResampleContext *c, struct AudioData *dst, int dst_size, struct AudioData *src, int src_size, int *consumed);
typedef int     (* resample_flush_func)(struct SwrContext *c);
typedef int     (* set_compensation_func)(struct ResampleContext *c, int sample_delta, int compensation_distance);
typedef int64_t (* get_delay_func)(struct SwrContext *s, int64_t base);
typedef int     (* invert_initial_buffer_func)(struct ResampleContext *c, struct AudioData *dst, const struct AudioData *src, int src_size, int *dst_idx, int *dst_count);
...
struct SwrContext {
    ...
    struct AudioData in;                                   ///< input audio data
    struct AudioData postin;                               ///< post-input audio data: used for rematrix/resample
    struct AudioData midbuf;                               ///< intermediate audio data (postin/preout)
    struct AudioData preout;                               ///< pre-output audio data: used for rematrix/resample
    struct AudioData out;                                  ///< converted output audio data
    struct AudioData in_buffer;                            ///< cached audio data (convert and resample purpose)
    struct AudioData silence;                              ///< temporary with silence
    struct AudioData drop_temp;                            ///< temporary used to discard output
...
int swri_realloc_audio(struct AudioData *a, int count);

void swri_noise_shaping_int16 (SwrContext *s, struct AudioData *dsts, const struct AudioData *srcs, const struct AudioData *noises, int count);
void swri_noise_shaping_int32 (SwrContext *s, struct AudioData *dsts, const struct AudioData *srcs, const struct AudioData *noises, int count);
void swri_noise_shaping_float (SwrContext *s, struct AudioData *dsts, const struct AudioData *srcs, const struct AudioData *noises, int count);
void swri_noise_shaping_double(SwrContext *s, struct AudioData *dsts, const struct AudioData *srcs, const struct AudioData *noises, int count);

av_warn_unused_result
int swri_rematrix_init(SwrContext *s);
void swri_rematrix_free(SwrContext *s);
int swri_rematrix(SwrContext *s, struct AudioData *out, struct AudioData *in, int len, int mustcopy);
