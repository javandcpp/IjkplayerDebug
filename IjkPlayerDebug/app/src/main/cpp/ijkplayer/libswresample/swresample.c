...
static void set_audiodata_fmt(struct AudioData *a, enum AVSampleFormat fmt){
    a->fmt   = fmt;
...
static void free_temp(struct AudioData *a){
    av_free(a->data);
    memset(a, 0, sizeof(*a));
}
...
static void copy(struct AudioData *out, struct AudioData *in,
                 int count){
...
static void fill_audiodata(struct AudioData *out, uint8_t *in_arg [SWR_CH_MAX]){
    int i;
    if(!in_arg){
...
static void reversefill_audiodata(struct AudioData *out, uint8_t *in_arg [SWR_CH_MAX]){
    int i;
    if(out->planar){
        for(i=0; i<out->ch_count; i++)
            in_arg[i]= out->ch[i];
 ...
 * out may be equal in.
 */
static void buf_set(struct AudioData *out, struct AudioData *in, int count){
    int ch;
    if(in->planar){
        for(ch=0; ch<out->ch_count; ch++)
 ...
 */
static int resample(SwrContext *s, struct AudioData *out_param, int out_count,
                             const struct AudioData * in_param, int in_count){
    struct AudioData in, out, tmp;
    int ret_sum=0;
    int border=0;
...
static int swr_convert_internal(struct SwrContext *s, struct AudioData *out, int out_count,
                                                      struct AudioData *in , int  in_count){
    struct AudioData *postin, *midbuf, *preout;
    int ret/*, in_max*/;
    struct AudioData preout_tmp, midbuf_tmp;
...
//     in_count= FFMIN(in_count, in_in + 2 - s->hist_buffer_count);
    ...
    if(preout != out && out_count){
        struct AudioData *conv_src = preout;
        if(s->dither.method){
            int ch;
...
int attribute_align_arg swr_convert(struct SwrContext *s, uint8_t *out_arg[SWR_CH_MAX], int out_count,
    ...
    struct AudioData * in= &s->in;
    struct AudioData *out= &s->out;
