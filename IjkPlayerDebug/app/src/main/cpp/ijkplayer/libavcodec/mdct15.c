...
static inline void fft5(struct FFTComplex *out, struct FFTComplex *in, struct FFTComplex exptab[2])
{
    struct FFTComplex z0[4], t[6];
...
static void fft15_c(struct FFTComplex *out, struct FFTComplex *in, struct FFTComplex *exptab, ptrdiff_t stride)
{
    int k;
    struct FFTComplex tmp1[5], tmp2[5], tmp3[5];
...
static void mdct15(MDCT15Context *s, float *dst, const float *src, ptrdiff_t stride)
{
    ...
    struct FFTComplex fft15in[15];

    /* Folding and pre-reindexing */
    for (i = 0; i < l_ptwo; i++) {
        for (j = 0; j < 15; j++) {
            const int k = s->pfa_prereindex[i*15 + j];
            struct FFTComplex tmp, exp = s->twiddle_exptab[k >> 1];
            if (k < len4) {
...
{
    struct FFTComplex fft15in[15];
    struct FFTComplex *z = (struct FFTComplex *)dst;
    ...
    for (i = 0; i < l_ptwo; i++) {
        for (j = 0; j < 15; j++) {
            const int k = s->pfa_prereindex[i*15 + j];
            struct FFTComplex tmp = { in2[-k*stride], in1[k*stride] };
...
static void postrotate_c(struct FFTComplex *out, struct FFTComplex *in, struct FFTComplex *exp,
