...
void ff_mdct15_postreindex_sse3(struct FFTComplex *out, struct FFTComplex *in, struct FFTComplex *exp, int *lut, ptrdiff_t len8);
void ff_mdct15_postreindex_avx2(struct FFTComplex *out, struct FFTComplex *in, struct FFTComplex *exp, int *lut, ptrdiff_t len8);

void ff_fft15_avx(struct FFTComplex *out, struct FFTComplex *in, struct FFTComplex *exptab, ptrdiff_t stride);

static void perm_twiddles(MDCT15Context *s)
{
    int k;
    struct FFTComplex tmp[30];
    ...
    for (k = 0; k < 6; k++) {
        ...
        memcpy(s->exptab + 8*k, ac_exp, 8*sizeof(struct FFTComplex));
    }

    /* Specialcase when k = 0 */
    for (k = 0; k < 3; k++) {
        struct FFTComplex dc_exp[] = {
            { tmp[2*k + 0].re, -tmp[2*k + 0].im },
            { tmp[2*k + 0].im,  tmp[2*k + 0].re },
            { tmp[2*k + 1].re, -tmp[2*k + 1].im },
            { tmp[2*k + 1].im,  tmp[2*k + 1].re },
        };
        memcpy(s->exptab + 8*6 + 4*k, dc_exp, 4*sizeof(struct FFTComplex));
    }
}
