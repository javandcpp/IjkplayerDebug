 ...
 * It also assumes all struct FFTComplex are 8 bytes-aligned pairs of floats.
 */

#if HAVE_VSX
#include "fft_vsx.h"
#else
void ff_fft_calc_altivec(FFTContext *s, struct FFTComplex *z);
void ff_fft_calc_interleave_altivec(FFTContext *s, struct FFTComplex *z);
#endif
...
#if HAVE_VSX
    ff_fft_calc_vsx(s, (struct FFTComplex*)output);
#else
    ff_fft_calc_altivec(s, (struct FFTComplex*)output);
#endif
