...
static void fft_ref(struct FFTComplex *tabr, struct FFTComplex *tab, int nbits)
{
    int i, j;
    int n  = 1 << nbits;
    int n2 = n >> 1;
...
static inline void fft_permute(FFTContext *s, struct FFTComplex *z)
{
#if AVFFT
    av_fft_permute(s, z);
#else
...
static inline void fft_calc(FFTContext *s, struct FFTComplex *z)
{
#if AVFFT
    av_fft_calc(s, z);
#else
...
{
    struct FFTComplex *tab, *tab1, *tab_ref;
    FFTSample *tab2;
    ...
    tab      = av_malloc_array(fft_size, sizeof(struct FFTComplex));
    tab1     = av_malloc_array(fft_size, sizeof(struct FFTComplex));
    tab_ref  = av_malloc_array(fft_size, sizeof(struct FFTComplex));
...
#endif /* CONFIG_MDCT */
    case TRANSFORM_FFT:
        memcpy(tab, tab1, fft_size * sizeof(struct FFTComplex));
        fft_permute(s, tab);
        fft_calc(s, tab);
