...
typedef struct MDCT15Context {
    ...
    FFTContext ptwo_fft;
    struct FFTComplex *tmp;
    struct FFTComplex *twiddle_exptab;

    DECLARE_ALIGNED(32, struct FFTComplex, exptab)[64];

    /* 15-point FFT */
    void (*fft15)(struct FFTComplex *out, struct FFTComplex *in, struct FFTComplex *exptab, ptrdiff_t stride);

    /* PFA postrotate and exptab */
    void (*postreindex)(struct FFTComplex *out, struct FFTComplex *in, struct FFTComplex *exp, int *lut, ptrdiff_t len8);
