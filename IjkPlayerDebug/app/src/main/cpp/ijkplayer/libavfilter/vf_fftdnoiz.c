...
typedef struct FFTdnoizContext {
    ...
    void (*import_row)(struct FFTComplex *dst, uint8_t *src, int rw);
    void (*export_row)(struct FFTComplex *src, uint8_t *dst, int rw, float scale, int depth);
} FFTdnoizContext;
...
static void import_row8(struct FFTComplex *dst, uint8_t *src, int rw)
{
    int j;
...
static void export_row8(struct FFTComplex *src, uint8_t *dst, int rw, float scale, int depth)
{
    int j;
...
static void import_row16(struct FFTComplex *dst, uint8_t *srcp, int rw)
{
...
static void export_row16(struct  FFTComplex *src, uint8_t *dstp, int rw, float scale, int depth)
{
...
static int config_input(AVFilterLink *inlink)
{
    ...
    for (i = 0; i < s->nb_planes; i++) {
        ...
        p->buffer_linesize = p->b * p->nox * sizeof(struct  FFTComplex);
                         ...
                         float *buffer, int buffer_linesize, int plane)
{
    ...
    const int data_linesize = p->data_linesize / sizeof(struct  FFTComplex);
    struct  FFTComplex *hdata = p->hdata;
    struct  FFTComplex *vdata = p->vdata;
    int x, y, i, j;
    ...
    for (y = 0; y < noy; y++) {
        for (x = 0; x < nox; x++) {
            ...
            struct  FFTComplex *ssrc, *dst = hdata;
            ...
            for (i = 0; i < block; i++) {
                ...
                av_fft_permute(p->fft, dst);
                av_fft_calc(p->fft, dst);
                memcpy(bdst, dst, block * sizeof(struct  FFTComplex));
                         ...
                         float *buffer, int buffer_linesize, int plane)
{
    ...
    for (y = 0; y < noy; y++) {
        for (x = 0; x < nox; x++) {
            ...
            struct  FFTComplex *hdst, *ddst = vdata;

            hdst = hdata;
            for (i = 0; i < block; i++) {
                memcpy(ddst, bsrc, block * sizeof(struct  FFTComplex));
