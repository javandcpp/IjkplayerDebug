...
static int headphone_fast_convolute(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ...
    int offset = jobnr;
    int *write = &td->write[jobnr];
    struct  FFTComplex *hrtf = s->data_hrtf[jobnr];
    int *n_clippings = &td->n_clippings[jobnr];
    float *ringbuffer = td->ringbuffer[jobnr];
    const int ir_len = s->ir_len;
    const float *src = (const float *)in->data[0];
    ...
    const int buffer_length = s->buffer_length;
    const uint32_t modulo = (uint32_t)buffer_length - 1;
    struct  FFTComplex *fft_in = s->temp_fft[jobnr];
...
static int convert_coeffs(AVFilterContext *ctx, AVFilterLink *inlink)
{
    ...
    if (s->type == TIME_DOMAIN) {
        ...
        s->temp_fft[0] = av_calloc(s->n_fft, sizeof(struct  FFTComplex));
        s->temp_fft[1] = av_calloc(s->n_fft, sizeof(struct  FFTComplex));
    ...
    if (s->type == TIME_DOMAIN) {
        ...
        s->data_hrtf[0] = av_calloc(n_fft * s->nb_irs, sizeof(struct  FFTComplex));
        s->data_hrtf[1] = av_calloc(n_fft * s->nb_irs, sizeof(struct  FFTComplex));
        ...
        memcpy(s->data_hrtf[0], data_hrtf_l,
            sizeof(struct  FFTComplex) * nb_irs * n_fft);
