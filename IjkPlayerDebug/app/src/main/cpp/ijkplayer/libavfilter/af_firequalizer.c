...
static void fast_convolute2(FIREqualizerContext *av_restrict s, const float *av_restrict kernel_buf, struct FFTComplex *av_restrict conv_buf,
                            OverlapIndex *av_restrict idx, float *av_restrict data0, float *av_restrict data1, int nsamples)
{
    if (nsamples <= s->nsamples_max) {
        struct FFTComplex *buf = conv_buf + idx->buf_idx * s->rdft_len;
        struct FFTComplex *obuf = conv_buf + !idx->buf_idx * s->rdft_len + idx->overlap_idx;
        int center = s->fir_len/2;
        int k;
        float tmp;
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    if (!s->min_phase) {
        for (ch = 0; ch + 1 < inlink->channels && s->fft_ctx; ch += 2) {
            fast_convolute2(s, s->kernel_buf, (struct  FFTComplex *)(s->conv_buf + 2 * ch * s->rdft_len),
