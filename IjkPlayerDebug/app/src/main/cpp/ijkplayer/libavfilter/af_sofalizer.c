...
static int sofalizer_fast_convolute(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ...
    int offset = jobnr;
    int *write = &td->write[jobnr];
    struct  FFTComplex *hrtf = s->data_hrtf[jobnr]; /* get pointers to current HRTF data */
    int *n_clippings = &td->n_clippings[jobnr];
    float *ringbuffer = td->ringbuffer[jobnr];
    const int n_samples = s->sofa.n_samples; /* length of one IR */
    const float *src = (const float *)in->data[0]; /* get pointer to audio input buffer */
    ...
    for (i = 0; i < n_conv; i++) {
        ...
        memset(fft_in, 0, sizeof(struct  FFTComplex) * n_fft);
        ...
        for (j = 0; j < n_fft; j++) {
            const struct  FFTComplex *hcomplex = hrtf_offset + j;
...
static int load_data(AVFilterContext *ctx, int azim, int elev, float radius, int sample_rate)
{
    ...
    struct  FFTComplex *data_hrtf_l = NULL;
    struct  FFTComplex *data_hrtf_r = NULL;
    struct  FFTComplex *fft_in_l = NULL;
    struct  FFTComplex *fft_in_r = NULL;
    ...
    if (s->type == TIME_DOMAIN) {
        ...
        s->temp_fft[0] = av_malloc_array(s->n_fft, sizeof(struct  FFTComplex));
        s->temp_fft[1] = av_malloc_array(s->n_fft, sizeof(struct  FFTComplex));
    ...
    if (s->type == FREQUENCY_DOMAIN) {
        s->data_hrtf[0] = av_malloc_array(n_fft * s->n_conv, sizeof(struct  FFTComplex));
        s->data_hrtf[1] = av_malloc_array(n_fft * s->n_conv, sizeof(struct  FFTComplex));
        ...
        memcpy(s->data_hrtf[0], data_hrtf_l, /* copy HRTF data to */
            sizeof(struct  FFTComplex) * n_conv * n_fft); /* filter struct */
        memcpy(s->data_hrtf[1], data_hrtf_r,
            sizeof(struct  FFTComplex) * n_conv * n_fft);
    }
