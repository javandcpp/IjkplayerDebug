...
typedef struct ConvolveContext {
    ...
    struct  FFTComplex *fft_hdata[4];
    struct  FFTComplex *fft_vdata[4];
    struct  FFTComplex *fft_hdata_impulse[4];
    struct  FFTComplex *fft_vdata_impulse[4];
...
static int config_input_main(AVFilterLink *inlink)
{
    ...
    for (i = 0; i < s->nb_planes; i++) {
        ...
        if (!(s->fft_hdata[i] = av_calloc(s->fft_len[i], s->fft_len[i] * sizeof(struct  FFTComplex))))
            return AVERROR(ENOMEM);

        if (!(s->fft_vdata[i] = av_calloc(s->fft_len[i], s->fft_len[i] * sizeof(struct  FFTComplex))))
            return AVERROR(ENOMEM);

        if (!(s->fft_hdata_impulse[i] = av_calloc(s->fft_len[i], s->fft_len[i] * sizeof(struct  FFTComplex))))
            return AVERROR(ENOMEM);

        if (!(s->fft_vdata_impulse[i] = av_calloc(s->fft_len[i], s->fft_len[i] * sizeof(struct  FFTComplex))))
            return AVERROR(ENOMEM);
    }
...
static int fft_horizontal(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ...
    ThreadData *td = arg;
    struct  FFTComplex *hdata = td->hdata;
...
static void get_input(ConvolveContext *s, struct  FFTComplex *fft_hdata,
...
static int fft_vertical(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ...
    ThreadData *td = arg;
    struct  FFTComplex *hdata = td->hdata;
    struct  FFTComplex *vdata = td->vdata;
    const int plane = td->plane;
    const int n = td->n;
    int start = (n * jobnr) / nb_jobs;
    int end = (n * (jobnr+1)) / nb_jobs;
...
static void get_output(ConvolveContext *s, struct  FFTComplex *input, AVFrame *out,
...
static int complex_multiply(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ConvolveContext *s = ctx->priv;
    ThreadData *td = arg;
    struct  FFTComplex *input = td->hdata;
    struct  FFTComplex *filter = td->vdata;
...
static int do_convolve(FFFrameSync *fs)
{
    ...
    for (plane = 0; plane < s->nb_planes; plane++) {
        struct  FFTComplex *filter = s->fft_vdata_impulse[plane];
        struct  FFTComplex *input = s->fft_vdata[plane];
